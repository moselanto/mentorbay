        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {FEATURED_MENTORS.map((m) => (
            <Link key={m.id} href={`/mentors/${m.id}`} className="bg-white rounded-2xl shadow-card p-5 hover:-translate-y-1 transition block">
              ...
              <p className="text-xs text-slate-400 mt-3">{m.exp} Years Experience · {m.city}, {m.country}</p>
            </Link>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12">
          <div>
            <div className="flex items-end justify-between mb-6">
              ...
              <Link href="/events" className="text-teal-600 font-semibold text-sm hover:underline">View all events →</Link>
            </div>
              ...
              {UPCOMING_EVENTS.map((e) => (
                <Link key={e.id} href={`/events/${e.id}`} className="bg-white rounded-xl shadow-card p-4 flex items-center gap-4 hover:shadow-lg transition">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500 mt-1">{e.date} · {e.time} · {e.loc}</p>
                  </div>
          ...
          <div>
            <div className="flex items-end justify-between mb-6">
              ...
              <Link href="/programs" className="text-teal-600 font-semibold text-sm hover:underline">View all programs →</Link>
            </div>
              ...
              {POPULAR_PROGRAMS.map((p) => (
                <Link key={p.id} href={`/programs/${p.id}`} className="bg-white rounded-xl shadow-card overflow-hidden hover:shadow-lg transition block">
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-slate-500 mt-2">{p.weeks} · {p.level}</p>
                    <div className="flex items-center justify-between mt-3">
                      <span className="text-xs font-semibold text-teal-600 bg-teal-50 px-2 py-0.5 rounded">Free</span>
                      <span className="text-xs font-semibold text-navy">Enroll →</span>
                    </div>
                  </div>
                </Link>
              ))}
