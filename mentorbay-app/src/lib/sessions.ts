...
export type MySession = {
  id: string; topic: string; mode: string; when: string; counterpart: string; upcoming: boolean;
};

type Row = {
  id: string; topic: string; mode: string; scheduled_at: string; mentor_id: string | null; mentee_id: string;
...
export async function getMySessions(): Promise<MySession[]> {
  try {
    ...
    const { data, error } = await supabase
      .from("sessions")
      .select("id, topic, mode, scheduled_at, mentor_id, mentee_id, mentor:profiles!sessions_mentor_id_fkey(full_name), mentee:profiles!sessions_mentee_id_fkey(full_name)")
